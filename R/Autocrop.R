Autocrop <- function(mesh, max.len, max.itr=10000) {
  ...
  while (any(!is.na(new.tri)) & itr < max.itr) {
    itr <- itr + 1
    old.tri <- new.tri
    new.tri <- .ModTri(old.tri, mesh, max.len, elem.build)
  ...
  for (i in seq_along(arcs[,1])) {
    ...
    if (is.na(adjacent.arc[1])) return(NULL)

    if (node != adjacent.arc[1]) adjacent.arc <- rev(adjacent.arc)
    sort.arcs <- c(sort.arcs, adjacent.arc)
    arcs <- arcs[!has.shared.node, ]
    if (inherits(arcs, "numeric")) arcs <- t(as.matrix(arcs))
  }
