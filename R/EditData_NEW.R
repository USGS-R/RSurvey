...
EditData <- function(d, col.names=NULL, col.formats=NULL, read.only=FALSE,
  ...
  CallSearch <- function(is.replace=FALSE) {
    ...
    if (is.replace & (!is.null(matched.cells) && nrow(matched.cells) > 0)) {
      ...
      for (j in unique(ij[, 2])) {
        idxs <- which(ij[, 2] == j)
        ij.in.col <- ij[idxs, , drop=FALSE]
        d[ij.in.col] <<- as(new.vals[idxs], class(d[ij.in.col]))
      }

      e <- data.frame(time=Sys.time(), cell=cells, old=old.vals,
  ...
  ValidateCellValue <- function(s, S) {
    ...
    old.val <- as.character(s)
    new.val <- as.character(S)
    obj.class <- class(d[i, j])[1]

    if (inherits(d[i, j], c("numeric", "integer", "logical")) &&
        !identical(new.val, CheckEntry(obj.class, new.val))) {
      is.valid <- FALSE
    } else {
      e <- data.frame(time=Sys.time(), cell=cell, old=old.val, new=new.val,
                      stringsAsFactors=FALSE)
      d[i, j] <<- as(new.val, obj.class)
      undo.stack <<- rbind(undo.stack, e)
      redo.stack <<- NULL
      is.valid <- TRUE
    }
  ...
  UndoEdit <- function() {
    ...
    for (j in unique(ij[, 2])) {
      e.idxs <- which(ij[, 2] == j)
      e.ij <- ij[e.idxs, , drop=FALSE]
      d[e.ij] <<- as(vals[e.idxs], class(d[e.ij]))
    }
