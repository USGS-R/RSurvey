ProcessData <- function(d, type="p", coerce.rows=NULL, ply=NULL, grid.res=list(x=NA, y=NA),
  ...
  if (type == "p") {
    ...
    is.x <- "x" %in% var.names
    is.y <- "y" %in% var.names
    if (is.x) d <- d[is.finite(d[, "x"]), ]
    if (is.y) d <- d[is.finite(d[, "y"]), ]
    ...
    if (is.ply && is.x && is.y) {
      ...
      for (i in seq_along(all.pts)) {
        pts <- all.pts[[i]]
        is.in <- point.in.polygon(point.x=d$x, point.y=d$y, pol.x=pts$x, pol.y=pts$y)
        is.in <- if (pts$hole) is.in != 1 else is.in != 0
        d <- d[is.in, ]
      }
    }

    if (nrow(d) == 0) stop("Range excludes all point data")
  }

  # process gridded data
  if (type == "g") {

    if (is.null(d$x) | is.null(d$y) | is.null(d$z)) return()
    ...
    xnum <- ynum <- 100
    if (!is.na(grid.res$x)) xnum <- as.integer(diff(xlim) / grid.res$x) + 1
    if (!is.na(grid.res$y)) ynum <- as.integer(diff(ylim) / grid.res$y) + 1
    ...
    if (!is.na(grid.mba$m)) m <- grid.mba$m
    if (!is.na(grid.mba$n)) n <- grid.mba$n
    if (!is.na(grid.mba$h)) h <- grid.mba$h
